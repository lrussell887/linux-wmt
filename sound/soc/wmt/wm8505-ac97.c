...
#define VT1613_TEST		0x5a
#define VT1613_TEST_DCOR	BIT(10)	/* ADC DC-offset removal */
#define VT1613_CTRL		0x5c
#define VT1613_CTRL_SMUTE_DIS	BIT(0)	/* Soft-mute, active low */
#define VT1613_CTRL_AMP3536_DIS	BIT(5)	/* Headphone output amplifier, active low */
#define VT1613_CTRL_AMP3941_DIS	BIT(7)	/* Speaker output amplifier, active low */
			    ...
			    struct snd_kcontrol *k, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_ac97_update_bits(priv->ac97, VT1613_CTRL, VT1613_CTRL_AMP3941_DIS, 0);
		gpiod_set_value_cansleep(priv->speaker_gpio, 1);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		gpiod_set_value_cansleep(priv->speaker_gpio, 0);
		snd_ac97_update_bits(priv->ac97, VT1613_CTRL, VT1613_CTRL_AMP3941_DIS,
				     VT1613_CTRL_AMP3941_DIS);
		break;
	}
	return 0;
}
...
/*
 * wm8505_hp_event - DAPM event handler for the headphone amplifier
 */
static int wm8505_hp_event(struct snd_soc_dapm_widget *w,
			   struct snd_kcontrol *k, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_ac97_update_bits(priv->ac97, VT1613_CTRL, VT1613_CTRL_AMP3536_DIS, 0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_ac97_update_bits(priv->ac97, VT1613_CTRL, VT1613_CTRL_AMP3536_DIS,
				     VT1613_CTRL_AMP3536_DIS);
		break;
	}
	return 0;
}
...
static const struct snd_soc_dapm_widget wm8505_dapm_widgets[] = {
	...
	SND_SOC_DAPM_HP("Headphone", wm8505_hp_event),
};
...
static int wm8505_ac97_machine_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	struct wm8505_ac97 *priv = snd_soc_card_get_drvdata(rtd->card);

	priv->ac97 = snd_soc_component_get_drvdata(component);

	snd_ac97_update_bits(priv->ac97, VT1613_TEST, VT1613_TEST_DCOR, VT1613_TEST_DCOR);
	snd_ac97_update_bits(priv->ac97, VT1613_CTRL, VT1613_CTRL_SMUTE_DIS, 0);
