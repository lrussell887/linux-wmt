...
/*
 * Hardware Descriptor Format 1 (16 bytes)
 * Employs branching logic necessary for continuous cyclic transfers.
 */
...
/*
 * Context RAM Block (Memory Register)
 * The DMA controller acts as an initiator to fetch its runtime context
 * directly from this contiguous structure in physical memory.
 */
...
/*
 * Software Descriptor
 * Wraps the virt-dma node and the underlying hardware descriptor.
 */
struct wmt_dma_desc {
...
struct wmt_dma_chan {
	...
	u32                      id;
	u32                      req_id; /* Maps to the peripheral's hardware request line */
	int                      irq;
...
/**
 * wmt_dma_alloc_chan_resources - allocate channel resources
 */
static int wmt_dma_alloc_chan_resources(struct dma_chan *chan)
{
	return 0;
}

/**
 * wmt_dma_free_chan_resources - free all virt-dma channel resources
 */
...
/**
 * wmt_dma_device_config - DMA device configuration callback
 */
...
/**
 * wmt_dma_free_desc - release a software descriptor and its hardware resources
 */
...
/**
 * wmt_dma_start_desc - commit descriptor to the controller and start transfer
 */
static void wmt_dma_start_desc(struct wmt_dma_chan *c)
{
	...
	/* Interface 0 (IF0) always points to the dynamic memory descriptor chain */
	dmadev->mem_regs[c->id].if0_cpr = desc->hw_desc_phys;

	/* Interface 1 (IF1) always points to the static peripheral FIFO address */
	...
	/* Activate hardware processing and enforce Context RAM write-back */
	writel(desc->ccr_value | WMT_DMA_CCR_SYSTEM_RUN | WMT_DMA_CCR_UP_MEMREG_EN, c->ccr_reg);
}

/**
 * wmt_dma_issue_pending - start pending DMA transfers
 */
...
/**
 * wmt_dma_get_ccr - build a channel control register value from device config
 */
static u32 wmt_dma_get_ccr(struct wmt_dma_chan *c, enum dma_transfer_direction dir)
{
	...
	/* Bind peripheral Request ID */
	ccr |= (c->req_id << WMT_DMA_CCR_REQ_ID_SHIFT) & WMT_DMA_CCR_REQ_ID_MASK;
	return ccr;
}

/**
 * wmt_dma_prep_dma_cyclic - prepare a cyclic DMA descriptor chain
 */
							       ...
							       unsigned long flags)
{
	...
	/* Prevent buffer overflow against the descriptor DMA pool */
	if (periods > WMT_DMA_MAX_DESC_CNT) {
		dev_err(chan->device->dev, "Too many periods requested (%zu)\n", periods);
		return NULL;
	}
	...
	for (i = 0; i < periods; i++) {
		...
		/* Enable hardware interrupt upon completion of this specific period */
...
/**
 * wmt_dma_prep_slave_sg - prepare a scatter-gather DMA descriptor chain
 */
							     ...
							     unsigned long flags, void *context)
{
	...
	/* Pre-calculate necessary hardware descriptors considering max transfer size limits */
	for_each_sg(sgl, sg, sg_len, i)
		num_descs += DIV_ROUND_UP(sg_dma_len(sg), WMT_DMA_DES_REQCNT_MASK);

	if (num_descs == 0 || num_descs > WMT_DMA_MAX_DESC_CNT) {
		dev_err(chan->device->dev, "Invalid SG descriptor count: %u\n", num_descs);
		return NULL;
	}
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		while (len) {
			size_t chunk = min_t(size_t, len, WMT_DMA_DES_REQCNT_MASK);
	...
	/* Fixup the final hardware descriptor in the chain */
	desc->hw_desc[d - 1].req_cnt |= WMT_DMA_DES_END;
	if (flags & DMA_PREP_INTERRUPT)
		desc->hw_desc[d - 1].req_cnt |= WMT_DMA_INTEN_DES;
...
/**
 * wmt_dma_terminate_all - terminate all pending DMA transfers on a channel
 */
static int wmt_dma_terminate_all(struct dma_chan *chan)
{
	...
	writel(readl(c->ccr_reg) & ~WMT_DMA_CCR_SYSTEM_RUN, c->ccr_reg);
...
/**
 * wmt_dma_synchronize - virt-dma wrapper
 */
...
/**
 * wmt_dma_tx_status - report DMA transfer status and residue
 */
					 ...
					 struct dma_tx_state *txstate)
{
	...
	if (c->active_desc && c->active_desc->vdesc.tx.cookie == cookie) {
		...
		bool found = false;
		/*
		 * Read the memory-side address directly from the coherent
		 * Context RAM block. IF0 always tracks the memory address
		 * regardless of transfer direction.
		 */
		u32 current_dar = dmadev->mem_regs[c->id].if0_dar;
		int i;

		/* Scan the descriptor chunks to isolate the active transfer window */
...
/**
 * wmt_dma_interrupt - per-channel DMA interrupt handler
 */
static irqreturn_t wmt_dma_interrupt(int irq, void *dev_id)
{
	...
	spin_lock_irqsave(&c->vchan.lock, flags);
	if (c->active_desc) {
		if (evt == WMT_DMA_EVT_SUCCESS || evt == 0) {
			if (c->active_desc->cyclic) {
				...
				/* Dequeue and fire the next SG transfer immediately */
			...
			/* Halts execution gracefully upon catastrophic AHB/FIFO fault */
			dev_err(dmadev->ddev.dev, "Hardware Fault. Channel %d event: %d\n",
				c->id, evt);
			writel(ccr & ~WMT_DMA_CCR_SYSTEM_RUN, c->ccr_reg);
		}
	}
	spin_unlock_irqrestore(&c->vchan.lock, flags);
...
/**
 * wmt_dma_xlate - OF DMA translator to bind a request ID to a channel
 */
...
/**
 * wmt_dma_probe - platform device probe
 */
static int wmt_dma_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(dmadev->clk))
		return dev_err_probe(dev, PTR_ERR(dmadev->clk), "Failed to initialize clock\n");

	/* Allocate 512 bytes of coherent memory for the Context RAM block */
	...
	/* Software reset to guarantee clean state */
	writel(WMT_DMA_GCR_SW_RST, dmadev->base + WMT_DMA_GCR);

	/* Wake hardware out of dormant state */
	writel(WMT_DMA_GCR_GLOBAL_EN, dmadev->base + WMT_DMA_GCR);

	/* Bind the physical address of the Context RAM to the DMA Controller */
	writel(dmadev->mem_regs_phys, dmadev->base + WMT_DMA_MRPR);

	/* Globally unmask all 16 hardware channel interrupts */
	writel(0xffff, dmadev->base + WMT_DMA_IER);

	/* Initialize memory pool for 16-byte Format 1 hardware descriptors */
	...
	dmadev->ddev.dev = dev;
	dmadev->ddev.device_alloc_chan_resources = wmt_dma_alloc_chan_resources;
	...
	for (i = 0; i < WMT_DMA_CHANNELS; i++) {
		...
		ret = devm_request_irq(dev, irq, wmt_dma_interrupt, 0, dev_name(dev), c);
		if (ret)
			return ret;
	}
...
/**
 * wmt_dma_remove - platform device remove
 */
