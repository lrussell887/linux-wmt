...
/**
 * wm8505_ps2_wait_ibf - Wait for input buffer empty
 */
...
/**
 * wm8505_ps2_wait_obf - Wait for output buffer full
 */
...
/**
 * wm8505_ps2_flush - Flush the data register
 */
...
/**
 * wm8505_ps2_write_ctr - Write to the control register
 */
...
/**
 * wm8505_ps2_hw_init - Initialize hardware state
 */
static int wm8505_ps2_hw_init(struct wm8505_ps2 *ps2)
{
	...
	if ((readl(ps2->base + WM8505_KBDC_DATA) & 0xff) != KBDC_RET_SELF_TEST)
		dev_warn(ps2->dev, "controller self-test failed\n");
	...
	ps2->ctr |= I8042_CTR_KBDDIS | I8042_CTR_AUXDIS | I8042_CTR_XLATE;
	ps2->ctr &= ~(I8042_CTR_KBDINT | I8042_CTR_AUXINT);

	return wm8505_ps2_write_ctr(ps2, ps2->ctr);
}

/**
 * wm8505_ps2_interrupt - Interrupt handler
 */
static irqreturn_t wm8505_ps2_interrupt(int irq, void *dev_id)
{
	...
	/* Ignore spurious error flags to prevent loss of valid packets */
	if (status & I8042_STR_AUXDATA)
		serio_interrupt(ps2->aux_port, data, 0);
	else
		serio_interrupt(ps2->kbd_port, data, 0);

	return IRQ_HANDLED;
}

/**
 * wm8505_ps2_open - Open a serio port
 */
...
/**
 * wm8505_ps2_close - Close a serio port
 */
static void wm8505_ps2_close(struct serio *port)
{
	...
	wm8505_ps2_write_ctr(ps2, ctr);
	spin_unlock_irqrestore(&ps2->lock, flags);
}

/**
 * wm8505_ps2_kbd_write - Write to the keyboard port
 */
...
/**
 * wm8505_ps2_aux_write - Write to the auxiliary port
 */
...
/**
 * wm8505_ps2_allocate_port - Allocate and configure a serio port
 */
...
/**
 * wm8505_ps2_probe - Probe platform device
 */
...
/**
 * wm8505_ps2_remove - Remove platform device
 */
