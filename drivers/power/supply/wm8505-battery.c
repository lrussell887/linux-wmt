...
/*
 * wm8505_battery_curve_uv - Battery microvolts from the rise curve
 *
 * Evaluates V = Vth / (1 - e^(-x)), x = t/tau in Q20. Pade(3,3) for e^(-x)
...
/*
 * wm8505_battery_rise_to_uv - Convert a rise time to battery microvolts
 *
 * The anchor rise was captured at a known voltage; any rise converts by
 * ratio to it.
 */
...
/*
 * wm8505_battery_solve_anchor - Bisect the rise curve for the alarm voltage
 *
 * Where the alarm voltage sits on the curve follows from the constants alone.
 */
...
/*
 * wm8505_battery_calibrate - Bank the rise time at the alarm edge
 *
 * A single rise at the known trip voltage fixes the unit's entire scale.
 */
...
/*
 * wm8505_battery_sample_us - Take a median-filtered rise sample
 */
...
/*
 * wm8505_battery_set_property - Restore or clear the banked alarm rise
 */
...
static int wm8505_battery_probe(struct platform_device *pdev)
{
	...
	/*
	 * Revision strap: tied high where charge-done completes at 0,
	 * low where it completes at 1.
	 */
