...
/*
 * wmt_ge_reset - Recover the GE after a hardware timeout
 */
...
/*
 * wmt_ge_set_dst - Program the destination window registers
 */
static void wmt_ge_set_dst(void __iomem *regs, dma_addr_t dst, struct wmt_ge_op *op)
{
	writel(dst, regs + WMT_GE_DES_BADDR);
	writel((op->dst_pitch / 4) - 1, regs + WMT_GE_DES_DISP_W);
...
/*
 * wmt_ge_fill - Program and fire a solid fill operation
 */
static void wmt_ge_fill(void __iomem *regs, dma_addr_t dst, struct wmt_ge_op *op)
{
...
/*
 * wmt_ge_blit - Program and fire a BitBlt operation
 */
...
static bool wmt_ge_validate_op(struct wmt_ge_op *op, u32 dst_size, u32 src_size)
{
	if (op->type == WMT_GE_OP_FILL)
		return (op->rop == WMT_GE_ROP_PAT_COPY || op->rop == WMT_GE_ROP_PAT_XOR) &&
		       wmt_ge_bounds_ok(dst_size, op->dst_pitch, op->dst_x, op->dst_y,
					op->width, op->height);
	if (op->type == WMT_GE_OP_BLIT)
		return (op->rop == WMT_GE_ROP_SRC_COPY || op->rop == WMT_GE_ROP_SRC_XOR) &&
		       wmt_ge_bounds_ok(src_size, op->src_pitch, op->src_x, op->src_y,
					op->width, op->height) &&
		       wmt_ge_bounds_ok(dst_size, op->dst_pitch, op->dst_x, op->dst_y,
					op->width, op->height);
	return false;
	...
	if (!wmt->ge_console && !wmt->ge_reset_pending) {
		if (flag & WMT_GE_INT_TIMEOUT) {
			...
			if (++job->op_cursor >= job->num_ops) {
				...
				/* Retire only at quiesce or as the ring nears full */
				retire = wmt->ge_tail == wmt->ge_head ||
...
int wmt_drm_ioctl_ge_wait(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
	...
	if (req->seqno) {
		spin_lock_irqsave(&wmt->ge_lock, flags);
		future = !ge_passed(wmt->ge_seq, req->seqno);
		spin_unlock_irqrestore(&wmt->ge_lock, flags);
		if (future)
			return -EINVAL;
